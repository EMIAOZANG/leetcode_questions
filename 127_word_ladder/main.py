...
class Solution(object):
    def edit_distance(self, str_1, str_2):
        ...
        for i in range(0,min(len(str_1),len(str_2))):
            sum += int(str_1[i] != str_2[i])
            if sum > 1:
                return False
        return sum == 1
            
    def BFS(self,m,word_list,target_str):
        '''
        using queue to implement BFS, return distance when found
        '''
        dist = 0
        q = deque([])
        visited = [False] * len(m) #visit mark
        if len(m) == 0:
            return 0
        ...
        while len(q) > 0:
            row_idx, dist = q.popleft()
            if word_list[row_idx] == target_str:
                return dist+1
            for col_idx in m[row_idx]:
                if visited[col_idx] == False:
                    #if edge exists and destination is not visited
                    visited[col_idx] = True
                    q.append((col_idx,dist+1))
        return 0

    def BFS_fast(self,beginWord,endWord,wordList):
        
        unvisited_set = wordList.copy()
        ...
        while len(q) > 0:
            word_from, dist = q.popleft()
            if self.edit_distance(word_from,endWord):
                return dist+2
            for word_to in unvisited_set.copy():
                if self.edit_distance(word_from,word_to):  #can be improved by enumerating 26 letters
                    unvisited_set.remove(word_to)
                    q.append((word_to,dist+1))
        return 0

    
    def BFS_fast_enum(self,beginWord,endWord,wordList):
        ...
        while len(q) > 0:
            ...
            if self.edit_distance(word_from,endWord):
                return dist+2
            for idx in range(len(word_from)):
                for i in range(26):=`=jedi=0, =`=               (*_*i*_*) =`=jedi=`=
                    new_word = word_from[:idx]+chr(ord('a')+i))+word_from[idx+1:]
                    if new_word in unvisited_set.copy():
                       unvisited_set.remove(new_word)
